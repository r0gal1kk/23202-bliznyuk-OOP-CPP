...
void WAVFileReader::read() {
    ...
    if (!file.is_open()) {
        throw FileIOException("Cannot open WAV file: " + filename);
    }
    ...
    if (std::strncmp(header.chunkID, "RIFF", 4) != 0 ||
        ...
        throw UnsupportedFormatException("Invalid WAV file format: " + filename);
    }
    ...
    if(header.audioFormat != 1) {
        throw UnsupportedFormatException("Must be no compress: " + filename);
    }
    file.read(reinterpret_cast<char *>(&header.numChannels), 2);
    if (header.numChannels != 1) {
        throw UnsupportedFormatException("Must be mono: " + filename);
    }
    file.read(reinterpret_cast<char *>(&header.sampleRate), 4);
    if (header.sampleRate != 44100) {
        throw UnsupportedFormatException("Must be 44100 hz: " + filename);
    }
    ...
    if(header.bitsPerSample != 16) {
        throw UnsupportedFormatException("Must be 16 bits per sample: " + filename);
    }
    if (std::strncmp(header.subchunk1ID, "fmt ", 4) != 0 || header.audioFormat
        != 1) {
        throw UnsupportedFormatException("Unsupported WAV format: " + filename);
    }
